...
class Adaptor{
  ...
  _init(){
    FlutterBleLib.instance.createClient(uuid);
  }
  open(callback){
    if (peripheral == null){
      ...
      //need to discover
      FlutterBleLib.instance.onStateChange().listen((state){
        if (state == BluetoothState.POWERED_ON){
          FlutterBleLib.instance.startDeviceScan(1, 1, uuid).listen((result) {
            FlutterBleLib.instance.stopDeviceScan();
            FlutterBleLib.instance.connectToDevice(uuid).then((device) =>_connectPeripheral(device,callback));
          });
  ...
  write(data,callback){
    ...
    writeServiceCharacteristic(RobotControlService,CommandsCharacteristic,data,callback);
  }
  onRead(callback){
  ...
  setTXPower(level,callback){
    writeServiceCharacteristic(BLEService,TXPowerCharacteristic,level,callback);
  }
  setAntiDos(callback){
    var str = "011i3";
    var bytes = [];
    ...
    writeServiceCharacteristic(BLEService,AntiDosCharacteristic,bytes,callback);
  }
  ...
  getCharacteristic(serviceId,characteristicId,callback){
    _connectBLE((){
     _connectService(serviceId,(error){
       ...
       _connectCharacteristic(serviceId,characteristicId,(e,c) => callback(e,c));
     });
    });
  }
  ...
  _connectService(serviceId,callback){
    var p = _connectedPeripheral();
    if (_connectedServices()){
      callback(null,_connectedService(serviceId));
    }else {
      FlutterBleLib.instance.servicesForDevice(uuid).then((services){
  ...
  _connectCharacteristic(serviceId, characteristicId, callback) {
    if (_connectedCharacteristics(serviceId)) {
      ...
      FlutterBleLib.instance.characteristicsForService(s).then((characteristics){
        if (characteristics.length > 0) {
          var characteristic = _connectedCharacteristic(serviceId,
              characteristicId);
  ...
  devModeOn(callback){
    setAntiDos(() {
      setTXPower(7, () {
        wake((){
          getCharacteristic(RobotControlService,ResponseCharacteristic,(e,c) {
            if (e) {
              return callback(e);
          }
            c.on("read",(data) {
