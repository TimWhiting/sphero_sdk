...
class Packet {
  ...
  parse(buffer) {
    if (this.partialBuffer.length > 0) {
      buffer = [this.partialBuffer, buffer];
  ...
  _parse(buffer) {
    ...
    packet['data'] = [for (var i = 0; i < packet['dlen'] - 1; i++) 0];
    // Copy data from buffer into packet.data
    buffer.copy(
        packet['data'], 0, FIELDS.size, FIELDS.size + packet['dlen'] - 1);
  ...
  _dealWithExtraBytes(buffer) {
    ...
    if (buffer.length > expectedSize) {
      this.partialBuffer = [for (var i = 0; i < expectedSize; i++) 0];
    } else {
      this.partialBuffer = [];
    }
  }
  ...
  _verifyChecksum(buffer, packet) {
    var bSlice =
            buffer.slice(FIELDS.mrspIdCode, FIELDS.checksum + packet.dlen - 1),
