...
class Core {
 ...
 /// @return {object} promise for command
  setDataStreaming(opts) {
    final n = opts.n.toHexArray( 2),
        m = opts.m.toHexArray( 2),
        mask1 = opts.mask1.toHexArray(4),
        pcnt = opts.pcnt &= 0xff,
        mask2 =opts.mask2.toHexArray( 4);
    ...
    final data = Uint8List.fromList([...n, ...m, ...mask1, pcnt, ...mask2]);

    return command(SpheroV1.setDataStreaming, data);
  }
 ...
 /// @return {object} promise for command
  configureLocator(opts) {
    final flags = opts.flags & 0xFF,
        x = opts.x.toHexArray( 2),
        y = opts.y.toHexArray( 2),
        yawTare = opts.yawTare.toHexArray( 2);

    final data = Uint8List.fromList([flags, ...x, ...y, ...yawTare]);

    return command(SpheroV1.locator, data);
  }
 ...
 /// @return {object} promise for command
  setAccelRange(idx) {
    idx &= idx;
    return command(SpheroV1.setAccelerometer, Uint8List.fromList([idx]));
  }
 ...
 /// @return {object} promise for command
  readLocator(callback) {
    return command(SpheroV1.readLocator, null);
  }
 ...
 /// @return {object} promise for command
  setRgbLed(opts) {
    final data = [opts.red, opts.green, opts.blue, opts.flag ?? 0x01];

    for (var i = 0; i < data.length; i++) {
      data[i] &= 0xFF;
    }

    return command(SpheroV1.setRgbLed, data);
  }
 ...
 /// @return {object} promise for command
  setBackLed(brightness) {
    return command(SpheroV1.setBackLed, Uint8List.fromList([brightness]));
  }
 ...
 /// @return {object} promise for command
  getRgbLed(callback) {
    return command(SpheroV1.getRgbLed, null);
  }
 ...
 /// @return {object} promise for command
  roll(speed, heading, state) {
    ...
    speed &= 0xFF;
    heading = toHexArray(heading, 2);
    state &= 0x03;

    final data = Uint8List.fromList(speed, heading, state);

    return command(SpheroV1.roll, data);
  }
 ...
 /// @return {object} promise for command
  boost(boost) {
    boost &= 0x01;
    return command(SpheroV1.boost, [boost]);
  }
 ...
 /// @return {object} promise for command
  setRawMotors(opts) {
    final lmode = opts.lmode & 0x07,
        lpower = opts.lpower & 0xFF,
        rmode = opts.rmode & 0x07,
        rpower = opts.rpower & 0xFF;

    final data = [lmode, lpower, rmode, rpower];

    return command(SpheroV1.setRawMotors, data);
  }
 ...
 /// @return {object} promise for command
  setMotionTimeout(time) {
    time = toHexArray(time, 2);
    return command(SpheroV1.setMotionTimeout, time);
  }
 ...
 /// }
 /// @return {object} promise for command
  setPermOptionFlags(flags) {
    flags = toHexArray(flags, 4);
    return command(SpheroV1.setOptionsFlag, flags);
  }
 ...
 /// @return {object} promise for command
  getPermOptionFlags(callback) {
    return command(SpheroV1.getOptionsFlag, null);
  }
 ...
 /// }
 /// @return {object} promise for command
  setTempOptionFlags(flags) {
    flags = toHexArray(flags, 4);
    return command(SpheroV1.setTempOptionFlags, flags);
  }
 ...
 /// @return {object} promise for command
  getTempOptionFlags(callback) {
    return command(SpheroV1.getTempOptionFlags, null);
  }
 ...
 /// @return {object} promise for command
  getConfigBlock(id) {
    id &= 0xFF;
    return command(SpheroV1.getConfigBlock, [id]);
  }

  _setSsbBlock(cmd, pwd, block) {
    pwd = pwd.toHexArray( 4);
    final data = Uint8List.fromList(pwd, block);
 ...
 /// }
 /// @return {object} promise for command
  setSsbModBlock(pwd, block) {
    return _setSsbBlock(SpheroV1.setSsbParams, pwd, block);
  }
 ...
 /// @return {object} promise for command
  setDeviceMode(mode) {
    mode &= 0x01;
    return command(SpheroV1.setDeviceMode, [mode]);
  }
 ...
 /// @return {object} promise for command
  getDeviceMode(callback) {
    return command(SpheroV1.getDeviceMode, null);
  }
 ...
 /// @return {object} promise for command
  getSsb(callback) {
    return command(SpheroV1.getSsb, null);
  }
 ...
 /// @return {object} promise for command
  setSsb(pwd, block) {
    return _setSsbBlock(SpheroV1.setSsb, pwd, block);
  }
 ...
 /// @return {object} promise for command
  refillBank(type) {
    type &= 0xFF;
    return command(SpheroV1.ssbRefill, [type]);
  }
 ...
 /// @return {object} promise for command
  buyConsumable(id, qty) {
    id &= 0xFF;
    qty &= 0xFF;
    return command(SpheroV1.ssbBuy, [id, qty]);
  }
 ...
 /// @return {object} promise for command
  useConsumable(id) {
    id &= 0xFF;
    return command(SpheroV1.ssbUseConsumeable, [id]);
  }
 ...
 /// }
 /// @return {object} promise for command
  grantCores(int pw, int qty, int flags) {
    pw = pw.toHexArray( 4);
    qty = qty.toHexArray( 4);
    flags &= 0xFF;

    final data = Uint8List.fromList(pw, qty, flags);

    return command(SpheroV1.ssbGrantCores, data);
  }

  _xpOrLevelUp(cmd, pw, gen) {
    pw = toHexArray(pw, 4);
    gen &= 0xFF;

    return command(cmd, Uint8List.fromList(pw, gen));
  }
 ...
 /// @return {object} promise for command
  addXp(pw, qty) {
    return _xpOrLevelUp(SpheroV1.ssbAddXp, pw, qty);
  }
 ...
 /// @return {object} promise for command
  levelUpAttr(pw, id) {
    return _xpOrLevelUp(SpheroV1.ssbLevelUpAttr, pw, id);
  }
 ...
 /// @return {object} promise for command
  getPasswordSeed(callback) {
    return command(SpheroV1.getPwSeed, null);
  }
 ...
 /// }
 /// @return {object} promise for command
  enableSsbAsyncMsg(flag) {
    flag &= 0x01;
    return command(SpheroV1.ssbEnableAsync, [flag]);
  }
 ...
 /// @return {object} promise for command
  runMacro(id) {
    id &= 0xFF;
    return command(SpheroV1.runMacro, [id]);
  }
 ...
 /// @return {object} promise for command
  saveTempMacro(macro) {
    return command(SpheroV1.saveTempMacro, macro);
  }
 ...
 /// @return {object} promise for command
  saveMacro(macro) {
    return command(SpheroV1.saveMacro, macro);
  }
 ...
 /// @return {object} promise for command
  reInitMacroExec(callback) {
    return command(SpheroV1.initMacroExecutive, null);
  }
 ...
 /// @return {object} promise for command
  abortMacro(callback) {
    return command(SpheroV1.abortMacro, null);
  }
 ...
 /// @return {object} promise for command
  getMacroStatus(callback) {
    return command(SpheroV1.macroStatus, null);
  }
 ...
 /// }
 /// @return {object} promise for command
  setMacroParam(index, val1, val2) {
    return command(
      SpheroV1.setMacroParam,
      argsToHexArray(index, val1, val2),
      callback
    );
  }
 ...
 /// }
 /// @return {object} promise for command
  appendMacroChunk(chunk) {
    return command(SpheroV1.appendTempMacroChunk, chunk);
  }
 ...
 /// @return {object} promise for command
  eraseOrbBasicStorage(area) {
    area &= 0xFF;
    return command(SpheroV1.eraseOBStorage, [area]);
  }
 ...
 /// }
 /// @return {object} promise for command
  appendOrbBasicFragment(area, code) {
    area &= 0xFF;
 ...
 /// }
 /// @return {object} promise for command
  executeOrbBasicProgram(area, slMSB, slLSB) {
    return command(
      SpheroV1.execOBProgram,
      argsToHexArray(area, slMSB, slLSB),
      callback
    );
  }
 ...
 /// @return {object} promise for command
  abortOrbBasicProgram(callback) {
    return command(SpheroV1.abortOBProgram, null);
  }
 ...
 /// }
 /// @return {object} promise for command
  submitValueToInput(val) {
    val = toHexArray(val, 4);
    return command(SpheroV1.answerInput, val);
  }
 ...
 /// @return {object} promise for command
  commitToFlash(callback) {
    return command(SpheroV1.commitToFlash, null);
  }

  _commitToFlashAlias(callback) {
    return command(SpheroV1.commitToFlashAlias, null);
  }
}
